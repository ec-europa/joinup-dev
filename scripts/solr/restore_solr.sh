...
function show_help {
 ...
 echo -e "-s    --snapshot-dir  The directory containing the snapshot"
...
while [[ "$1" == -* ]]; do
 case "$1" in
   ...
   -s|--snapshot-dir) shift; SNAPSHOT_DIR=$1; shift;;
   -s|--snapshot-name) shift; SNAPSHOT_NAME=$1; shift;;
...
CORE_EXISTS=`curl -sS "http://localhost:${PORT}/solr/admin/cores?action=STATUS&core=${CORE}" |grep -o '<long name="uptime">'`
...
# Restore de index.
line="http://localhost:8983/solr/${CORE}/replication?command=restore&name=${SNAPSHOT_NAME}&location=${SNAPSHOT_DIR}"
...
do
   ...
   CHECK_SUCCESS=`/usr/bin/curl -sS "http://localhost:8983/solr/${CORE}/replication?command=restorestatus&name=${SNAPSHOT_NAME}&location=${SNAPSHOT_DIR}"`
