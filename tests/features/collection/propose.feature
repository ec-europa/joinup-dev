@api @group-a
Feature: Proposing a community
  In order to create a new community on Joinup
  As the product owner of a community of software solutions
  I need to be able to propose a community for inclusion on Joinup

  # An anonymous user should be shown the option to add a community, so that
  # the user will be aware that communities can be added by the public, even
  # though you need to sign in to do so.
  Scenario: Anonymous user needs to sign in before creating a community
    Given users:
      | Username      | E-mail           |
  ...
  Scenario: Propose a community
    ...
    And I should see "Thank you for proposing a community. Your request is currently pending approval by the site administrator."

    # The user that proposed the community should be auto-subscribed.
    And the "Ancient and Classical Mythology" community should have 1 active member
    ...
    When I fill in the following:
      ...
      | Body  | <p>Some more<em>information</em><br />about the community.<p> |
    And I press "Save"
    Then I should see the success message "Custom page About this group has been created."
    And the page should contain the html text "<p>Some more<em>information</em><br>about the community.</p>"

    # Clean up the community that was created.
    Then I delete the "Ancient and Classical Mythology" community
    And I delete the "Contact person" contact information

  Scenario: Propose a community with a duplicate name
  ...
  Scenario: Propose community form fields should be organized in tabs.
    ...
    # As a user, editing a community, solution, event or news item, I want to
    # be able to pick-up a pre-uploaded image when editing the logo or the
    # banner, as an alternative to the image upload.
    When I select image #3 as community logo
    And I wait for AJAX to finish
    And I select image #8 as community banner
    And I wait for AJAX to finish
    And I press "Propose"
    Then I should see the success message "Thank you for proposing a community. Your request is currently pending approval by the site administrator."
    And the "Just a proposal" community logo is image #3
    And the "Just a proposal" community banner is image #8

    Given I go to the edit form of the "Just a proposal" community
