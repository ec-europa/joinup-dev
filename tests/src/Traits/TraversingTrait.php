...
trait TraversingTrait {
   ...
   *   The select option.
   * @param bool $remove_extra_spaces
   *   (optional) Remove multiple spaces from a string. Usually needed when
   *   multiple spaces appear in a string after stripping HTML tags.
   *
   * @throws \Exception
   *   Thrown if there is no selected option or the selected option is not the
   *   correct one.
   */
  protected function assertSelectedOption(NodeElement $element, string $expected, bool $remove_extra_spaces = FALSE): void {
   ...
   *    identical.
   */
  protected function assertSelectAvailableOptions(NodeElement $element, TableNode $table, bool $remove_extra_spaces = FALSE): void {
