...
interface GroupInterface extends ContentEntityInterface {
   ...
   * number of solutions which were migrated from the previous platform which
   * have multiple owners. Since we enforce a single owner when an ownership
   * transfer is initiated, the number of groups with multiple owners will
   * reduce over time. For the moment though we still need to account for
   * multiple owners.
   *
  ...
  /**
   * Returns whether the user with the given ID is the group owner.
   *
   * @param int $uid
   *   The user ID.
   *
   ...
   * In Joinup every new group has exactly one owner. However there are still a
   * number of solutions which were migrated from the previous platform which
   * have multiple owners. In some cases (such as transferring group ownership)
   * we need to be able to ascertain that a user is the sole owner of a group.
   *
   * @param int $uid
   *   The user ID.
   *
