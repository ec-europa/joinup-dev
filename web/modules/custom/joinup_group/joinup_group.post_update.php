...
/**
 * Migrate "Moderation" field data into the new "Content moderation" field.
 */
function joinup_group_post_update_migrate_moderation(): void {
  $predicate_mapping = [
    'http://joinup.eu/collection/moderation' => 'http://joinup.eu/collection/content_moderation',
    'http://joinup.eu/solution/moderation' => 'http://joinup.eu/solution/content_moderation',
  ];

  $value_mapping = [
    0 => ContentModerationOptions::NO_MODERATION,
    1 => ContentModerationOptions::MEMBERS_MODERATION,
  ];
...
WHERE {
  GRAPH ?graph {
    ?entity_id ?predicate ?value .
    VALUES ?predicate { <http://joinup.eu/collection/moderation> <http://joinup.eu/solution/moderation> } .
  }
}
Query;
  ...
  foreach ($results->getArrayCopy() as $result) {
    ...
    $graphs[$graph_uri]->add(
      $result->entity_id->getUri(),
      $predicate_mapping[$result->predicate->getUri()],
      $value_mapping[$result->value->getValue()]
    );
  }
  ...
  foreach ($graphs as $graph_uri => $graph) {
    // Delete the legacy moderation creation triples.
    $query = "WITH <{$graph_uri}>
DELETE { ?s ?p ?o . }
WHERE {
  ?s ?p ?o .
  VALUES ?p { <http://joinup.eu/collection/moderation> <http://joinup.eu/solution/moderation> } .
}";
    $sparql->query($query);
    $graph_store->insert($graph);
  }
