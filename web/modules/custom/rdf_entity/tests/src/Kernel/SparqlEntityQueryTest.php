...
class SparqlEntityQueryTest extends JoinupKernelTestBase {
  ...
  protected function setUp() {
    ...
    foreach ([$figures => 'shape', $greetings => 'text'] as $field_name => $field_type) {
      ...
      foreach ($field_storage->getPropertyNames() as $name) {
        $props[$name] = [
          'predicate' => 'http://' . $field_name . '.test/' . $name,
          'format' => 'literal',
        ];
      }
    ...
    for ($i = 0; $i < 2; $i++) {
      ...
      foreach ($rdf_third_party as $key => $value) {
        $type->setThirdPartySetting('rdf_entity', $key, $value);
      }
      $type->save();
  ...
  public function testEntityQuery() {
    ...
    $this->queryResults = $this->factory->get('rdf_entity')
      // @todo Exists doesn't work when it is the only condition.
      // @todo: THIS NOW WORKS.
      ->exists($greetings)
      // @todo Composite property conditions don't work, as blank nodes are not supported (yet).
      ->condition("$figures.color", 'red')
      ->sort('id')
      ->execute();
    // As unit 0 was the red triangle and unit 2 was the turkish greeting,
    // bit 0 and bit 2 needs to be set.
    // @todo Switch to this result once all conditions are implemented.

    // @todo This is the expected result:
    //$this->assertResult('http://entity/5', 'http://entity/7', 'http://entity/13', 'http://entity/15');
    // @todo But for some reason we get this:
    $this->assertResult('http://entity/005', 'http://entity/009', 'http://entity/013');
    ...
    // @todo This wrongfully returns 19 instead of 12:
    // Two reasons:
    // - default condition is added in a separate condition group,
    // matching more triples.
    // - The language filter is not applied.

    //$this->assertEqual(12, $count_query->count()->execute());
    $this->assertEqual(19, $count_query->count()->execute());
    ...
    $ids = $this->factory->get('rdf_entity')
...
//    ];
//
//    $this->queryResults = array_values($this->factory->get('rdf_entity')
//      ->tableSort($header)
//      ->execute());
...
//    $result = \Drupal::entityQuery('rdf_entity')->condition(
...
//    $result = $this->factory->get('rdf_entity')
//      ->condition('id', [14], 'IN')
//      ->condition("$this->figures.color", 'red')
//      ->allRevisions()
//      ->execute();
