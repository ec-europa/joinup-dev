...
class JoinupSubscriberServiceTest extends KernelTestBase {
  ...
  public function testGetSubscribers() {
    ...
    // Get subscribers as emails.
    $subscribers_mails = $subscription_service->getSubscribersAsMails($entity, 'follow');

    // Check the result counts.
    $this->assertCount(3, $subscribers_mails);

    // Check subscribed users based on their email.
    $this->assertEquals($this->users[0]->getEmail(), array_shift($subscribers_mails));
    $this->assertEquals($this->users[1]->getEmail(), array_shift($subscribers_mails));
    $this->assertEquals($this->users[2]->getEmail(), array_shift($subscribers_mails));
  }
