...
class Prepare extends DestinationBase implements MigrateDestinationFastRollbackInterface, ContainerFactoryPluginInterface {
  ...
  public function import(Row $row, array $old_destination_id_values = []) {
    ...
    if ($insert) {
      ...
      $found = (bool) $this->database->select('joinup_migrate_prepare', 'c')
        ->fields('c', ['collection'])
    ...
    try {
      // Inserting.
      if ($insert) {
        $this->database->insert('joinup_migrate_prepare')
      ...
      // Updating.
        ...
        $this->database->update('joinup_migrate_prepare')
          ->fields($values)
  ...
  public function rollback(array $destination_identifier) {
    ...
    $this->database->delete('joinup_migrate_prepare')
  ...
  public function rollbackAll() {
    $this->database->truncate('joinup_migrate_prepare')->execute();
  }
