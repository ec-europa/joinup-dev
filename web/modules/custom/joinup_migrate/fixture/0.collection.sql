...
SELECT
  ...
  IF(p.nid IS NULL, CONCAT('../resources/migrate/collection/logo/', p.logo), IF(n.type = 'community' AND fc.filepath IS NOT NULL AND fc.filepath <> '', fc.fid, IF(fr.filepath IS NOT NULL AND fr.filepath <> '', fr.fid, NULL))),
  IF(p.nid IS NULL, CONCAT('../resources/migrate/collection/logo/', p.logo), IF(n.type = 'community' AND fc.filepath IS NOT NULL AND fc.filepath <> '', SUBSTRING(fc.filepath, 21), IF(fr.filepath IS NOT NULL AND fr.filepath <> '', SUBSTRING(fr.filepath, 21), NULL))),
