...
SELECT
  ...
  nr.body,
  IF(m.logo IS NOT NULL, CONCAT('../resources/migrate/solution/logo/', m.logo), IF(m.type = 'asset_release' AND fl.filepath IS NOT NULL AND fl.filepath <> '', fl.fid, IF(fpl.filepath IS NOT NULL AND fpl.filepath <> '', fpl.fid, NULL))),
