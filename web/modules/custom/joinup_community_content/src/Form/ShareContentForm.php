...
class ShareContentForm extends FormBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(RdfEntitySparqlStorage $rdf_storage, RdfEntityViewBuilder $rdf_builder, JoinupRelationManager $relation_manager, MembershipManagerInterface $membership_manager, AccountInterface $current_user, ElementInfoManagerInterface $element_info) {
  ...
  /**
   * Process callback for the collection checkboxes.
   *
   * Runs after \Drupal\Core\Render\Element\Checkboxes::processCheckboxes()
   * has expanded the "checkboxes" element to single "checkbox" elements.
   */
  public function processCheckboxes(&$element, FormStateInterface $form_state, &$complete_form) {
    foreach (array_keys($element['#options']) as $key) {
      // Transform each checkbox into a submit-enabled input.
      $element[$key]['#executes_submit_callback'] = TRUE;
      // Set the id of the wrapper that is generated by
      // CompositeFormElementTrait::preRenderCompositeFormElement().
      $element[$key]['#ajax']['wrapper'] = $element['#id'] . '--wrapper';
    }
